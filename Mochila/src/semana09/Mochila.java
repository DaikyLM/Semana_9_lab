...
public class Mochila extends JFrame implements ActionListener {
	...
	void PrecioModelo(){
		if(mod=="Sherman") {pre=149.9;}
		else if(mod=="Faguo") {pre=89.9;}
		else if(mod=="Aldo") {pre=119.9;}
		else{pre=174.9;}
	}
	void ImporteCompra(){
		impC=pre*cant;
	...
	void ImporteDescuento(){
		if(cant<5) {impD=impC*0;}
		if(cant>=5 && cant<7) {impD=impC*0;}
		if(cant>=7 && cant<9) {impD=impC*0;}
		else{impD=impC*0.09;}
	}
	void ImportePagar(){
		impP=impC-impD;
	}
	void ObsequioChocolate(){
		if(impP<200) {choco=0;}
		if(impP>=200 && impP<500) {choco=1*cant;}
		if(impP>=500 && impP<700) {choco=2*cant;}
		else{choco=3*cant;}
	}
